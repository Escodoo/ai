...
class TestChatter(common.TransactionCase):
    ...
    def test_chat_html_body_is_comment(self):
        """HTML replies stay Discuss comments and are not re-wrapped as plaintext."""
        with mock.patch("requests.post") as mock_post:
            mock_post.return_value = mock.Mock(
                status_code=200,
        ...
        # message_post may escape a str body; do not wrap that again in <p>.
        self.assertNotIn("<p>&lt;p&gt;", ai_message.body)

    def test_chat_html_body_is_html_kwarg(self):
        """message_post body_is_html must still reach Odoo unchanged."""
        with mock.patch("requests.post") as mock_post:
            mock_post.return_value = mock.Mock(
                status_code=200,
                json=lambda: {
        ...
        self.assertNotIn("&lt;p&gt;", ai_message.body)
